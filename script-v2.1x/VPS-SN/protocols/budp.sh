...
activado() {
	msg -bar
	#puerto local
	[[ "$(ps x | grep badvpn | grep -v grep | awk '{print $1}')" ]] && msg -verd "                  ACTIVATED SUCCESSFULLY" || msg -ama "                 Falló"
	msg -bar
}
BadVPN() {
	...
	if [[ ! -e /bin/badvpn-udpgw ]]; then
		wget -O /bin/badvpn-udpgw https://raw.githubusercontent.com/khaledagn/sinnombre-vps-master_English_Official/master/LINKS-LIBRARIES/badvpn-udpgw &>/dev/null
	...
	echo -e "$(msg -verd "[2]")$(msg -verm2 "➛ ")$(msg -azu "AGGREGATE +PORT BADVPN ")"
	echo -e "$(msg -verd "[3]")$(msg -verm2 "➛ ")$(msg -azu "STOP SERVICE BADVPN")"

	msg -bar
	read -p "Type an option (default 1): " -e -i 1 portasx
	tput cuu1 && tput dl1
	if [[ ${portasx} = 1 ]]; then
		...
		msg -bar
		msg -tit
		msg -ama "          BADVPN DEACTIVATOR (UDP)"
		msg -bar
