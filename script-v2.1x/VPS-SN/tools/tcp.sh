...
start_menu() {
	...
	msg -bar
	msg -tit
	echo -e " TCP Acceleration (BBR/Plus) ${Red_font_prefix}By @SinNombre${Font_color_suffix}
 $(msg -bar)
  ...
  ${Green_font_prefix}[ 0 ]${Font_color_suffix} Exit script" && msg -bar
	...
	msg -bar
	read -p "$(echo -e "\e[31m► ${bla}Select An Option [0-3]:${amarillo}") " num
	case "$num" in
	0) ;;
	1) startbbr ;;
	2) startbbrplus ;;
